...
const AdminLogin: React.FC<AdminLoginProps> = ({ onLogin, onBack }) => {
  ...
  return (
    <div className="flex flex-col justify-center items-center bg-gray-950 px-4 min-h-screen font-sans text-gray-100">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-900 shadow-2xl p-8 border border-gray-800 rounded-2xl w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="font-bold text-white text-2xl">Admin Portal</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="bg-gray-800 px-4 py-2.5 border border-gray-700 focus:border-brand-gold rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-gold w-full text-white transition" />
          </div>
          <div>
            <label className="block mb-1 text-gray-400 text-sm">Password</label>
            <input type="password" required value={password} onChange={e => setPassword(e.target.value)} className="bg-gray-800 px-4 py-2.5 border border-gray-700 focus:border-brand-gold rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-gold w-full text-white transition" />
          </div>
